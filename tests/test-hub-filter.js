...
ok("👻 Toàn hình thuộc Tiện ích", map && map.HubInvis_Panel === "Tiện ích");
ok("🔐 Anti Ban thuộc Server", map && map.HubAntiBan_Panel === "Server");
ok("Admin/Explorer/Spy/Định vị không có khung 🦘/✨/🚀/👻",
  map && ![...move, "HubGlow_Panel", "HubInvis_Panel"].some((k) => ["Admin","Explorer","Spy","Định vị"].includes(map[k])));
ok("khung 🦘/✨/🚀/👻 không thuộc Server",
  map && ![...move, "HubGlow_Panel", "HubInvis_Panel"].some((k) => map[k] === "Server"));
...
ok("chip Di chuyển hiện tune/fly/speed/highjump/safe, ẩn glow/invis",
  ...
  !vis("Di chuyển","HubGlow_Panel") && !vis("Di chuyển","HubInvis_Panel"));
ok("chip Tiện ích hiện glow + toàn hình, ẩn bay/nhảy",
  vis("Tiện ích","HubGlow_Panel") && vis("Tiện ích","HubInvis_Panel") &&
